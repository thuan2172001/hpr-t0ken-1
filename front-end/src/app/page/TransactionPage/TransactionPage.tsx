            ...
            setTransactionHistory(data.list.reverse().map((element:any, i:number) =>  
                <tr className="row" key={i}>
                    <td className="col-4 hash-tag text-truncate"><span>
                        <a className="transaction-field" href={`https://scan.testnet.tomochain.com/txs/${element.transactionHash}`}>{element.transactionHash}</a></span></td>
                    <td className="col-2 hash-tag text-truncate"><span>
                        {element.event}</span></td>
                    <td className="col-2 hash-tag text-truncate"><span>
                        <a className="transaction-field"href={`https://scan.testnet.tomochain.com/address/${element.from}`}>{element.from}</a></span></td>
                    <td className="col-2 hash-tag text-truncate"><span>
                        <a className="transaction-field" href={`https://scan.testnet.tomochain.com/address/${element.to}`}>{element.to}</a></span></td>
                    <td className="col-2 hash-tag text-truncate"><span>
